...
class AI(Agent):
    ...
    def binary_action_from_shobu_action(self, binary_board:np.ndarray, player_id:int, shobu_action:tuple):
        ...
        for action in shobu_action:
            ...
            mask_r_passive = 1 << (passive_stone + length*direction)
            mask_r_active = 1 << (active_stone + length*direction)

            # Add a pushing action if needed (no other check since its already been done in the shobu action generation)
            if mask_active & binary_board[self.next_player(player_id), active_board]:
                mask_p_active = 1 << (active_stone + (length+1)*direction)
            else:
                mask_p_active = 0
            ...
            actions.append((xy_passive, xy_active, direction, length, passive_board, passive_stone, active_board, active_stone))
    ...
    def print_mask_action(self, xy_action:tuple):
        ...
        """
        print("Mask :")
        self.print_mask(self.mask_object.masks[xy_action[0], xy_action[1]])
        print("Mask Result :")
        self.print_mask(self.mask_object.mask_results[xy_action[0], xy_action[1]])
        print("Mask Origin :")
        self.print_mask(self.mask_object.mask_origins[xy_action[0], xy_action[1]])
        print("Mask Push :")
        self.print_mask(self.mask_object.mask_push_result[xy_action[0], xy_action[1]])
    ...
    def print_actions_masks(self, actions:list):
        ...
        for action in actions:
            ...
            print("Mask Passive : ")
            self.print_mask_action(action[0])
            print("Mask Active: ")
            self.print_mask_action(action[1])
    ...
    class BinaryMasks:
        ...
        def binary_mask_filter(self, binary_board:np.ndarray, player_id:int, passive_board:int, passive_stone:int, active_board:int, active_stone:int):
            ...
            for direction_id in range(8):
                for length_id in range(1, 3):
                    ...
                    if binary_board[ennemy_player_id, passive_board] & self.masks[x_passive, y_passive, direction_id, length_id] or binary_board[player_id, passive_board] & self.masks[x_passive, y_passive, direction_id, length_id]:
                        continue
                    ...
                    check_push_block = push_block_ennemy.bit_count() > 1 or push_block_ally.bit_count() > 1
        ...
        def binary_apply_action(self, binary_board:np.ndarray, player_id:int, action_mask_tuple:tuple):
            ...
            if length+1 <3 and (binary_board[self.next_player(player_id), active_board] & self.masks[xy_active[0], xy_active[1], direction, length]):
                new_board[self.next_player(player_id), active_board] |= self.mask_results[xy_active[0], xy_active[1], direction, length+1]
            return new_board
    ...
    def binary_apply_action_to_state(self, binary_board:np.ndarray, player_id:int, action_mask_tuple:tuple):
        ...
        new_actions = self.mask_object.binary_mask_actions(new_board, new_to_move)
    ...
    def binary_aplha_beta_search(self, binary_state:tuple, remaining_time:float):
        ...
        """
        action_id, action = self.binary_max_value(binary_state, -float("inf"), float("inf"), 0, remaining_time)
        return action_id
    ...
    def binary_max_value(self, binary_state:tuple, alpha:float, beta:float, depth:int, remaining_time:float):
        ...
        if self.binary_iscutoff(binary_board, player_id, depth) == 1: # TODO : HACKcheck if that's the condition we should be checking !
            return self.mask_object.binary_heuristic_evaluation(binary_board, player_id), None
        ...
        for action_id, action in enumerate(binary_state[3]):
            ...
            test_value, _ = self.binary_min_value(test_state, max_value, beta, depth + 1, remaining_time)
            print("Max value 3 : ", max_value)
    ...
    def binary_min_value(self, binary_state:tuple, alpha:float, beta:float, depth:int, remaining_time:float):
        ...
        binary_board, player_id, _, actions = binary_state
        # Check if we have to cut off the search TODO add the time condition 
        if self.binary_iscutoff(binary_board, player_id, depth) != 0:
            return self.mask_object.binary_heuristic_evaluation(binary_board, player_id), None
        ...
        for action_id, action in enumerate(binary_state[3]):
            test_state = self.binary_apply_action_to_state(binary_board, player_id, action)
            print("Min value 1 : ", min_value)
            test_value, _ = self.binary_max_value(test_state, min_value, beta, depth + 1, remaining_time)
            print("action : ", action)
            self.print_binary_board(test_state[0])
